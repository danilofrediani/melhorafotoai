...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (String(purchaseToken).startsWith('FAKE_PURCHASE_TOKEN_')) {
      ...
      await fetch(consumeUrl, { method: "POST", headers: { Authorization: `Bearer ${accessToken}` } });
    }
    ...
    if (creditsToAdd > 0) {
      ...
      if (rpcError) {
        if ((rpcError as any).code === "42883") {
          console.error("[verify-play-purchase] RPC add_user_credits não existe");
        } else {
    ...
    return new Response(JSON.stringify({ ok: true, creditsAdded: creditsToAdd, sku, orderId, simulated: isSimulated }), {
